...
if grau_decimal == "ERRO_MINUTOS" or grau_decimal is None:
    st.stop()
...
def get_planetary_data(ano_ref, grau_ref_val, analisar_lua, mes_unico, long_natal_ref):
    ...
    for jd in steps:
        ...
        for p in planetas_cfg:
            ...
            # Aumentamos o símbolo usando tags HTML span para destaque
            simbolo_render = f"<span style='font-size: 24px;'> &nbsp;{simbolo}</span>" if simbolo else ""
            ...
            row[f"{p['nome']}_info"] = f"{get_signo(res[0])} {'(R)' if res[3]<0 else '(D)'} {int(pos):02d}°{int((pos%1)*60):02d}' - {'Forte' if dist <= 1.0 else 'Médio' if dist <= 2.5 else 'Fraco'} {simbolo_render}"
        all_data.append(row)
        ...
        for i in range(1, len(serie) - 1):
            if serie[i] > 0.98 and serie[i] > serie[i-1] and serie[i] > serie[i+1]:
                ...
                eventos_aspectos.append({
                    ...
                    "Planeta e Signo em Trânsito": f"{p['nome'].capitalize()} em {get_signo(row_pico[f'{p[ 'nome']}_long'])}",
                    "Trânsito": row_pico[f"{p['nome']}_status"],
