...
# 1. Seletor do Planeta (ORDEM ATUALIZADA)
dict_planetas = {
    "SOL": swe.SUN, 
    "LUA": swe.MOON, 
...
for p in natal_config_base:
    ...
    natal_final.append({"nome": f"{p['id']} ({novo_grau}°)", "grau": dms_to_dec(novo_grau), "cor": nova_cor})
...
for p in natal_final:
    fig.add_trace(go.Scatter(x=df['date'], y=df[p['nome']], mode='lines', name=p['nome'],
                             line=dict(color=p['cor'], width=2.5), fill='tozeroy', 
    ...
    for _, row in peaks.iterrows():
        fig.add_annotation(x=row['date'], y=row[p['nome']], text=row['date'].strftime('%d/%m'),
                           font=dict(color=p['cor'], size=10, family="Arial Black"),
                           showarrow=True, arrowhead=1, ay=-20, bgcolor="white")

fig.update_layout(xaxis=dict(rangeslider=dict(visible=True, thickness=0.05), type='date'),
...
# --- BOTÃO DE DOWNLOAD ---
# Convertendo o gráfico para HTML em memória
buffer = io.StringIO()
...
st.download_button(
    ...
    data=html_bytes,
    file_name=f"transitos_{planeta_nome.lower()}_{ano}.html",
    mime="text/html"
)